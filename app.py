...
if st.session_state.get("personality_saved", False) or (
    ...
    Помни: ты не бот, а реальный человек. Твои ответы должны быть такими, чтобы ни у кого не возникло сомнений в этом.
    """
    ...
    if st.button("📝 Получить фидбек о моём стиле общения", key="feedback_btn", 
        ...
        fb_prompt = f"""
        Ты — эксперт по коммуникациям и онлайн-знакомствам. Твоя задача — дать развернутый, честный, но поддерживающий фидбек по переписке ниже. 
        
        1. Прочитай диалог внимательно. Оцени каждое сообщение пользователя с точки зрения психологии дейтинга, учитывая современные исследования о том, что делает общение в дейтинге привлекательным.
        2. Составь три пункта:
          1) **Что в сообщениях пользователя кажется привлекательным** — укажи, что может заинтересовать или расположить собеседника. Опирайся на критерии: персонализация, искренний интерес, юмор, теплота, естественность, баланс между вопросами и рассказом о себе, уважение к границам.
          2) **Что может оттолкнуть собеседника** — выдели моменты, которые выглядят неестественно, слишком формально, навязчиво, шаблонно, агрессивно или могут создать впечатление “опроса”. Обращай внимание на избыточные вопросы, однотипность, слишком короткие или длинные ответы, отсутствие эмпатии или попыток понять собеседника.
          3) **Совет по следующему шагу** — порекомендуй конкретный стиль или тему сообщения, которая органично продолжит диалог. Совет должен опираться на лучшие практики дейтингового общения: быть тёплым, открытым, уместно персонализированным, без давления и излишних вопросов. Можно порекомендовать больше делиться историями, шутить, проявлять эмпатию или поддерживать инициативу, если это уместно.
        
        Всегда отвечай только на чистом русском языке, не используй ни одного иностранного слова или выражения, даже коротких англицизмов. Не вставляй описания своих действий или эмоций в скобках или звёздочках, просто пиши фидбек, как если бы ты давал его своему знакомому.
        
        Структурируй ответ понятно: три пункта, каждый с заголовком.
        
        Пиши доброжелательно и тактично — даже если есть ошибки, цель фидбека — помочь человеку стать привлекательнее и увереннее в общении.
        
        Перед тем, как дать фидбек, быстро, но вдумчиво проанализируй диалог и подумай, какие моменты реально могут работать, а какие — мешать. Не спеши.
